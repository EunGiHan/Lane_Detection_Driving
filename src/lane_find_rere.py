...
class Sliding:
    ...
    def sliding_win(self):
        ...
        self.out_img = np.dstack((self.lane, self.lane, self.lane))*255 # grayscale인 것을 rgb로 바꾸고자 3차원으로 쌓고, 0~1 사이의 값을 0~255로 바꿈
        ...
        for window in range(self.win_num):
            ...
            ### 왼, 오른쪽 슬라이딩 윈도우를 그림
            cv2.rectangle(self.out_img, (left_win_left_x, win_lower_y), (left_win_right_x, win_upper_y), (0, 255, 0), 2) # 왼차선 박스 그리기
            cv2.rectangle(self.out_img, (right_win_left_x, win_lower_y), (right_win_right_x, win_upper_y), (0, 255, 0), 2) # 오른차선 박스 그리기
        ...
        self.out_img[nz[0][left_lane_idx], nz[1][left_lane_idx]] = [255, 0, 0]   # 왼차선은 파란색
        self.out_img[nz[0][right_lane_idx], nz[1][right_lane_idx]] = [0, 0, 255]   # 오른차선은 빨간색
    ...
    def draw_line_to_src(self):
        ...
        ### 접선 구하기 & 그리기
        horizontal_pos = int(self.warp_img_height / 2 + 50)
        self.left_x = int(self.left_fit[0]*horizontal_pos**2 + self.left_fit[1]*horizontal_pos + self.left_fit[2])
        self.right_x = int(self.right_fit[0]*horizontal_pos**2 + self.right_fit[1]*horizontal_pos + self.right_fit[2])
        left_s = 2*self.left_fit[0]*horizontal_pos + self.left_fit[1]
        right_s = 2*self.right_fit[0]*horizontal_pos + self.right_fit[1]
        self.left_slope = math.degrees(math.tanh(2*self.left_fit[0]*horizontal_pos + self.left_fit[1]))    # 직선의 기울기(deg)
        self.right_slope = math.degrees(math.tanh(2*self.right_fit[0]*horizontal_pos + self.right_fit[1]))    # 직선의 기울기(deg)

        if self.left_slope * self.right_slope < 0:
            # 두 직선의 기울기가 반대면 이상한 모양임
            self.angle_err = 0
        else:
            self.mean_slope = (self.left_slope + self.right_slope) / 2  # 두 직선의 평균 기울기(deg)
            self.angle_err = -self.mean_slope
        ...
        # src_warp2 = cv2.warpPerspective(color_warp_line, self.perspec_mat_inv, (self.img_width, self.img_height))
        self.frame = cv2.addWeighted(self.frame, 1, src_warp, 0.3, 0)    # 원본에 0.3의 투명도로 다각형을 덧그림
        # self.frame = cv2.addWeighted(self.frame, 1, src_warp2, 1, 0)
...
class Drive:
    ...
    def angle_pid(self, angle_err):
        ...
        self.angle_p_err = angle_err
        self.angle_i_err += angle_err
    ...
    def pub_to_motor(self, angle_err):
        self.angle_pid(angle_err)
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if time_diff.secs > 0.5:
            drive.pub_to_motor(sliding.angle_err)
            print("="*30)
            print("left lane: {0}    right lane: {1}".format(sliding.left_x, sliding.right_x))
            print("left slope: {0:0.5f}  right slope: {1:0.5f}  mean slope: {2:0.5f}".format(sliding.left_slope, sliding.right_slope, sliding.mean_slope))
            print("angle error: {0:0.5f}  angle u: {1:0.5f}  steering: {2}".format(sliding.angle_err, drive.angle_u, drive.steer_angle))
            if drive.steer_angle < 0:
                print("Turn left")
            else:
                print("Turn right")
            print("\n\n")
